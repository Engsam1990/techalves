...
async function writeDb(db) {
    ...
    try {
        ...
        await runRuntimeDbMigrations(connection);
        await executeMany(connection, insertSql("admin_users", ["id", "email", "password", "name", "role", "permissions_json", "is_active", "created_at", "updated_at", "data_entrant", "entry_date"]), (db.adminUsers || []).map((item) => [
            item.id,
            item.email,
            item.password,
            item.name,
