...
async function persistAdminPermissionsJson(admin) {
    const permissionsJson = (0, auth_1.normalizeAdminRole)(admin.role) === "super_admin"
        ? null
        : JSON.stringify((0, auth_1.getAdminPermissionsForUser)(admin));
    await (0, db_1.executeDb)("UPDATE `admin_users` SET `permissions_json` = ?, `updated_at` = ? WHERE `id` = ?", [permissionsJson, (0, db_1.nowIso)().slice(0, 23).replace("T", " ").replace("Z", ""), admin.id]);
}
