...
router.get("/overview", (0, auth_1.requireAdminPermission)("dashboard:view"), async (_req, res, next) => {
    try {
...
router.get("/inventory-report", (0, auth_1.requireAdminPermission)("inventory:view"), async (req, res, next) => {
    try {
        const query = financeReportQuerySchema.parse(req.query);
        const range = buildFinanceRange(query);
        const db = await (0, db_1.readDb)();
