...
const DASHBOARD_OVERVIEW_PERMISSIONS = [
    "dashboard:view",
...
function requireDashboardOverviewPermission(req, _res, next) {
    try {
        if (DASHBOARD_OVERVIEW_PERMISSIONS.some((permission) => (0, auth_1.hasAdminPermission)(req.adminUser, permission))) {
            return next();
        }
...
const INVENTORY_REPORT_PERMISSIONS = [
    "inventory:view",
...
function requireInventoryReportPermission(req, _res, next) {
    try {
        if (INVENTORY_REPORT_PERMISSIONS.some((permission) => (0, auth_1.hasAdminPermission)(req.adminUser, permission))) {
            return next();
        }
        throw new errorHandler_1.AppError(403, "You do not have permission to perform this admin action");
    }
...
const financeReportQuerySchema = zod_1.z.object({
    days: zod_1.z.coerce.number().int().min(1).max(365).optional(),
    start: zod_1.z.string().trim().optional(),
    end: zod_1.z.string().trim().optional(),
});
...
function resolveBrandName(db, value) {
    ...
    const brand = (db.brands || []).find((item) => String(item.id || "").toLowerCase() === normalized ||
    ...
    if (looksLikeRawId(text))
        return null;
    return text;
}
function resolveOrderItemBrandName(db, item, product) {
    return (resolveBrandName(db, item?.productBrand) ||
...
function buildPaymentBreakdown(orders) {
    ...
    for (const item of orders) {
        ...
        if (!grouped.has(key))
            grouped.set(key, { status: key, count: 0, value: 0 });
...
function formatFinanceOrderRows(db, orders) {
    return (0, db_1.sortByDateDesc)(orders).map((order) => {
...
function formatIssueOrders(orders) {
    return (0, db_1.sortByDateDesc)(orders)
        .slice(0, 5)
        .map((order) => ({
        id: order.id,
        deliveryName: order.deliveryName,
...
function buildPendingIssues(orders) {
    ...
    const missingReferenceOrders = activeOrders.filter((item) => item.paymentStatus === "paid" &&
        transactionReferenceRequiredMethods.has(String(item.paymentMethod || "")) &&
        !String(item.transactionReference || "").trim());
    const paidUnfulfilledOrders = activeOrders.filter((item) => item.paymentStatus === "paid" &&
...
function formatExpenseRows(db, expenses) {
    return (0, db_1.sortByDateDesc)(expenses.map((expense) => ({ ...expense, createdAt: expenseDateValue(expense) || expense.createdAt }))).map((expense) => {
...
function formatSourcedProducts(db, products) {
    return (0, db_1.sortByDateDesc)(products.map((product) => ({ ...product, createdAt: productSourceDateValue(product) || product.createdAt }))).map((product) => {
...
function buildFinanceReport(db, range, req = null) {
    const canViewAll = !req || (0, auth_1.hasAdminPermission)(req.adminUser, "finance:view_all");
    const canSeeRecord = (record, ...extraRefs) => canViewAll || isOwnAdminRecord(req, record, ...extraRefs);
    const filteredOrders = (0, db_1.sortByDateDesc)(db.orders
        .filter((item) => isInRange(item.createdAt, range))
        .filter((item) => canSeeRecord(item))
    ...
    const filteredExpenses = (db.expenses || []).filter((item) => isInRange(expenseDateValue(item), range) && isActiveExpense(item) && canSeeRecord(item));
    ...
    const sourcedProductsInRange = (db.products || []).filter((product) => {
        ...
        return hasSource && isInRange(productSourceDateValue(product), range) && canSeeRecord(product, product.sourcingPaidBy);
    });
...
function classifyStockStatus(quantity) {
    ...
    if (stockQuantity <= lowStockThreshold)
        return "low_stock";
    if (stockQuantity >= overstockThreshold)
        return "overstock";
    return "healthy";
}
...
function buildInventoryReport(db, range) {
    const movements = (0, db_1.sortByDateDesc)(db.inventoryMovements.filter((item) => isInRange(item.createdAt, range)));
    ...
    const products = db.products.map((product) => createInventoryProductSnapshot(db, product, {
    ...
    const dormantProducts = products.filter((item) => item.stockQuantity > 0 &&
    ...
    const categoryBreakdown = [...db.categories, { id: "uncategorized", name: "Uncategorized" }]
        .map((category) => {
        const items = products.filter((product) => category.id === "uncategorized" ? !product.categoryId : product.categoryId === category.id);
        return {
    ...
    const recentMovements = movements.slice(0, 20).map((movement) => {
        const product = (0, db_1.findProductById)(db, movement.productId);
...
router.get("/overview", requireDashboardOverviewPermission, async (_req, res, next) => {
    try {
        ...
        const productInquiries = (0, db_1.sortByDateDesc)(db.contactMessages.filter((item) => isProductInquiry(item)));
        ...
        res.json({
            ...
            recentOrders: (0, db_1.sortByDateDesc)(db.orders)
                .slice(0, 5)
                .map((item) => ({
                id: item.id,
...
router.get("/finance-report", (0, auth_1.requireAdminPermission)("finance:view"), async (req, res, next) => {
    try {
        const query = financeReportQuerySchema.parse(req.query);
        const range = buildFinanceRange(query);
        const db = await (0, db_1.readDb)();
        res.json(buildFinanceReport(db, range, req));
    }
    catch (e) {
        next(e);
    }
});
router.get("/inventory-report", requireInventoryReportPermission, async (req, res, next) => {
    try {
        const query = financeReportQuerySchema.parse(req.query);
        const range = buildFinanceRange(query);
        const db = await (0, db_1.readDb)();
...
router.get("/products", (0, auth_1.requireAdminPermission)("analytics:view"), async (req, res, next) => {
    try {
...
router.get("/timeline", (0, auth_1.requireAdminPermission)("analytics:view"), async (req, res, next) => {
    try {
        const days = parseInt(String(req.query.days || 30), 10) || 30;
        const since = Date.now() - (days * 24 * 60 * 60 * 1000);
        const db = await (0, db_1.readDb)();
        const grouped = {};
        db.productAnalytics.filter((item) => new Date(item.createdAt).getTime() >= since).sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()).forEach((item) => { const date = String(item.createdAt).split("T")[0]; if (!grouped[date])
            grouped[date] = { date, views: 0, clicks: 0, inquiries: 0 }; if (item.event === "view")
            grouped[date].views += 1;
        else if (item.event === "click")
            grouped[date].clicks += 1;
        else if (item.event === "inquiry")
            grouped[date].inquiries += 1; });
