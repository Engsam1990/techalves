...
router.use((0, auth_1.requireAdminPermission)("orders:view"));
const listQuerySchema = zod_1.z.object({
    status: zod_1.z.string().optional(),
    paymentStatus: zod_1.z.string().optional(),
    paymentMethod: zod_1.z.string().optional(),
    financeIssue: zod_1.z.string().optional(),
...
const updateOrderSchema = zod_1.z.object({
    status: zod_1.z.enum(["pending", "confirmed", "processing", "completed", "delivered", "cancelled"]).optional(),
    paymentStatus: zod_1.z.enum(["pending", "partial", "paid", "failed", "refunded"]).optional(),
    transactionReference: zod_1.z.string().trim().max(120).optional().nullable(),
    itemSerialNumbers: zod_1.z.record(zod_1.z.array(zod_1.z.string().trim().min(1).max(191))).optional(),
...
const paymentEventSchema = zod_1.z.object({
    eventType: zod_1.z.enum(["payment", "partial_payment", "refund", "payment_status_change", "adjustment"]).default("payment"),
    amount: zod_1.z.coerce.number().min(0).default(0),
    paymentMethod: zod_1.z.enum(["cash_on_delivery", "mpesa", "bank_transfer", "cash", "card", "other"]).optional().nullable(),
    reference: zod_1.z.string().trim().max(120).optional().nullable(),
    note: zod_1.z.string().trim().max(500).optional().nullable(),
});
const returnItemSchema = zod_1.z.object({
    orderItemId: zod_1.z.string().trim().min(1),
    quantity: zod_1.z.coerce.number().int().min(1),
    serialNumbers: zod_1.z.array(zod_1.z.string().trim().min(1).max(191)).optional(),
});
const orderReturnSchema = zod_1.z.object({
    items: zod_1.z.array(returnItemSchema).min(1),
    restoreStock: zod_1.z.boolean().default(true),
    refundAmount: zod_1.z.coerce.number().min(0).default(0),
    refundMethod: zod_1.z.enum(["cash_on_delivery", "mpesa", "bank_transfer", "cash", "card", "other"]).optional().nullable(),
    reference: zod_1.z.string().trim().max(120).optional().nullable(),
...
function orderAlreadyHasStockDeduction(db, orderId) {
    return (db.inventoryMovements || []).some((movement) => String(movement.orderId || "") === String(orderId) &&
...
function orderMatchesFinanceIssue(db, order, issueKey) {
    ...
    if (!key)
        return true;
    if (key === "open")
        return buildOrderIssues(db, order).length > 0;
...
function assignOrderItemSerials(db, product, order, item, requestedSerialNumbers, soldAt, reservedSerialKeys = new Set()) {
    ...
    if (selectedSerials.length !== Number(item.quantity || 0)) {
        throw new errorHandler_1.AppError(400, `${product.name} requires ${item.quantity} serial number(s) before confirmation`);
    }
    const trackedByKey = new Map(trackedSerials.map((serial) => [(0, db_1.normalizeSerialNumber)(serial.serialNumber).toLowerCase(), serial]));
    const availableByKey = new Map((0, db_1.getAvailableSerialNumbers)(db, product.id)
        .filter((serial) => !reservedSerialKeys.has((0, db_1.normalizeSerialNumber)(serial.serialNumber).toLowerCase()))
        .map((serial) => [(0, db_1.normalizeSerialNumber)(serial.serialNumber).toLowerCase(), serial]));
    const assigned = [];
    for (const serialNumber of selectedSerials) {
        const serialKey = serialNumber.toLowerCase();
        if (reservedSerialKeys.has(serialKey)) {
            throw new errorHandler_1.AppError(400, `Serial number ${serialNumber} is already used in this order`);
        }
        const serial = availableByKey.get(serialKey);
        if (!serial) {
            if (trackedByKey.has(serialKey)) {
                throw new errorHandler_1.AppError(400, `Serial number ${serialNumber} has already been sold for ${product.name}`);
            }
...
router.get("/", async (req, res, next) => {
    try {
        ...
        let items = (0, db_1.sortByDateDesc)(db.orders).filter((item) => (!query.status || normalizeOrderStatus(db, item) === query.status) &&
        ...
        if (!(0, auth_1.hasAdminPermission)(req.adminUser, "orders:view_all")) {
        ...
        if (query.q) {
            items = items.filter((order) => {
                ...
                return [order.id, order.deliveryName, order.deliveryPhone, order.deliveryEmail, order.transactionReference, entrant.name, entrant.email, entrant.id, entrant.label, customer?.email, customer?.fullName]
                    .some((value) => (0, db_1.contains)(value, query.q));
            });
        }
...
router.get("/:id", async (req, res, next) => {
    try {
        ...
        if (!(0, auth_1.hasAdminPermission)(req.adminUser, "orders:view_all") && !isOwnAdminRecord(req, order)) {
            throw new errorHandler_1.AppError(403, "You can only view orders tied to your admin activity");
        }
...
router.post("/:id/payment-events", async (req, res, next) => {
    try {
        ...
        if (!(0, auth_1.hasAdminPermission)(req.adminUser, requiredPermission)) {
        ...
        if (!order)
            throw new errorHandler_1.AppError(404, "Order not found");
        if (!(0, auth_1.hasAdminPermission)(req.adminUser, "orders:view_all") && !isOwnAdminRecord(req, order)) {
            throw new errorHandler_1.AppError(403, "You can only update orders tied to your admin activity");
        }
        if (normalizeOrderStatus(db, order) === "cancelled" && data.eventType !== "refund") {
            throw new errorHandler_1.AppError(400, "Only refund records can be added to cancelled orders");
        }
        if (["payment", "partial_payment"].includes(data.eventType) && Number(data.amount || 0) <= 0) {
            throw new errorHandler_1.AppError(400, "Payment amount must be greater than zero");
        }
        if (data.eventType === "refund" && Number(data.amount || 0) <= 0) {
            throw new errorHandler_1.AppError(400, "Refund amount must be greater than zero");
        }
        if (paymentNeedsTransactionReference(data.paymentMethod || order.paymentMethod, data.eventType === "refund" ? "paid" : "paid") && !normalizeTransactionReference(data.reference)) {
            throw new errorHandler_1.AppError(400, "Transaction reference is required for this payment method");
        }
        appendPaymentEvent(order, data, req, db);
        updateOrderPaymentStatusFromEvents(order);
        if (data.eventType !== "refund" && data.paymentMethod)
            order.paymentMethod = data.paymentMethod;
        if (data.reference)
            order.transactionReference = normalizeTransactionReference(data.reference);
        order.updatedAt = (0, db_1.nowIso)();
...
router.post("/:id/returns", (0, auth_1.requireAdminPermission)("orders:return_items"), async (req, res, next) => {
    try {
        ...
        if (!["confirmed", "processing", "completed", "delivered"].includes(orderStatus)) {
            throw new errorHandler_1.AppError(400, "Only confirmed, processing, completed, or delivered orders can receive returned stock");
        }
        ...
        for (const returnItem of data.items) {
            ...
            if (!orderItem)
                throw new errorHandler_1.AppError(400, "Return contains an invalid order item");
            ...
            if (returnItem.quantity > remaining) {
                throw new errorHandler_1.AppError(400, `Cannot return ${returnItem.quantity} unit(s) for ${orderItem.productName || "item"}; only ${remaining} unit(s) remain returnable`);
            }
            ...
            if (existingSerials.length && requestedSerials.length !== returnItem.quantity) {
                throw new errorHandler_1.AppError(400, `${orderItem.productName || "Item"} requires ${returnItem.quantity} returned serial number(s)`);
            }
            for (const serial of requestedSerials) {
                if (!existingSerials.map((value) => value.toLowerCase()).includes(serial.toLowerCase())) {
                    throw new errorHandler_1.AppError(400, `Serial number ${serial} is not assigned to this order item`);
                }
            }
            ...
            if (data.restoreStock) {
                const product = (0, db_1.findProductById)(db, orderItem.productId);
                if (product) {
...
router.patch("/:id", (0, auth_1.requireAdminPermission)("orders:update_status"), async (req, res, next) => {
    try {
        ...
        if (isWalkInOrder(db, order) && nextStatus === "delivered") {
            throw new errorHandler_1.AppError(400, "Walk-in customer orders cannot be marked as delivered");
        }
        if (paymentNeedsTransactionReference(order.paymentMethod, nextPaymentStatus) && !nextTransactionReference) {
            throw new errorHandler_1.AppError(400, "Transaction reference is required before marking this payment as paid");
        }
        if (shouldDeductInventory(currentStatus, nextStatus)) {
            const serialAssignmentAt = (0, db_1.nowIso)();
            const reservedSerialKeys = new Set();
            const alreadyDeducted = orderAlreadyHasStockDeduction(db, order.id);
            for (const item of order.items || []) {
                const product = (0, db_1.findProductById)(db, item.productId);
                if (!product)
                    throw new errorHandler_1.AppError(400, "Order contains an invalid product");
                if (!alreadyDeducted && product.stockQuantity < item.quantity) {
                    throw new errorHandler_1.AppError(400, `Not enough stock to confirm order for ${product.name}`);
                }
        ...
        if (data.transactionReference !== undefined)
            order.transactionReference = nextTransactionReference;
        order.updatedAt = (0, db_1.nowIso)();
        await (0, db_1.writeDb)(db);
        res.json(formatOrder(db, order));
    }
