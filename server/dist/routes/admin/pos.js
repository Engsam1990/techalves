...
function assertAndReserveSerials(db, product, orderItem, serialNumbers, order, soldAt) {
    ...
    const availableByKey = new Map((0, db_1.getAvailableSerialNumbers)(db, product.id, { excludeOrderId: order.id }).map((item) => [(0, db_1.normalizeSerialNumber)(item.serialNumber).toLowerCase(), item]));
    const assigned = [];
