...
function canEditProductSection(req, permission) {
    if (permission === "sourcing_payments:update") {
        return (0, auth_1.hasAdminPermission)(req.adminUser, permission);
    }
...
const receiveStockSchema = zod_1.z.object({
    ...
    sourcedBy: zod_1.z.string().trim().max(191).optional().nullable(),
    sourceDate: zod_1.z.string().trim().max(20).optional().nullable(),
    paymentStatus: zod_1.z.enum(["pending", "paid", "partial", "waived"]).optional(),
...
router.get("/", (0, auth_1.requireAdminPermission)("products:view"), async (req, res, next) => {
    try {
        ...
        if (!(0, auth_1.hasAdminPermission)(req.adminUser, "products:view_all")) {
...
router.get("/:id", (0, auth_1.requireAdminPermission)("products:view"), async (req, res, next) => {
    try {
        ...
        if (!(0, auth_1.hasAdminPermission)(req.adminUser, "products:view_all") && !isOwnAdminRecord(req, product, product.sourcingPaidBy)) {
            throw new errorHandler_1.AppError(403, "You can only view products tied to your admin activity");
        }
