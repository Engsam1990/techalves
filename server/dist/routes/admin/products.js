...
const PRODUCT_SECTION_FIELD_PERMISSIONS = [
    ...
    { permission: "products:edit_sources", fields: ["sourceId", "sourcedFrom", "sourcedBy", "sourceDate", "sourcePrice", "sourcePaymentStatus", "sourcingPaymentStatus"] },
...
function requiredProductUpdatePermissions(body) {
    ...
    if (!required.size)
        required.add("products:edit");
    return Array.from(required);
}
function canEditProductSection(req, permission) {
    return (0, auth_1.hasAdminPermission)(req.adminUser, "products:edit") || (0, auth_1.hasAdminPermission)(req.adminUser, permission);
}
function requireProductUpdatePermission(req, _res, next) {
    try {
        ...
        if (missing.length) {
            throw new errorHandler_1.AppError(403, "You do not have permission to edit this product section");
        }
        next();
    }
    catch (error) {
...
function deleteRemovedProductImageFiles(db, productId, previousImages, nextImages) {
    ...
    for (const url of Array.isArray(previousImages) ? previousImages : []) {
        ...
        if (!imageUrl || nextSet.has(imageUrl) || !isUploadUrl(imageUrl))
            continue;
        if (isImageUsedByOtherProduct(db, productId, imageUrl))
            continue;
        (0, uploads_1.deleteUploadedFileByUrl)(imageUrl);
    }
}
const imageValueSchema = zod_1.z.string().min(1).max(1000);
const productSchema = zod_1.z.object({
    ...
    originalPrice: zod_1.z.number().int().min(0).optional().nullable(),
    condition: zod_1.z.enum(["new", "refurbished", "ex-uk"]),
    description: zod_1.z.string().min(1),
    specs: zod_1.z.record(zod_1.z.string()).default({}),
    warrantyText: zod_1.z.string().optional().nullable(),
    stockQuantity: zod_1.z.number().int().min(0).default(0),
    featured: zod_1.z.boolean().default(false),
    premium: zod_1.z.boolean().default(false),
    images: zod_1.z.array(imageValueSchema).default([]),
    serialNumbers: zod_1.z.array(zod_1.z.string().trim().min(1).max(191)).default([]),
    sourcedFrom: zod_1.z.string().trim().max(191).optional().nullable(),
    sourcedBy: zod_1.z.string().trim().max(191).optional().nullable(),
    sourceDate: zod_1.z.string().trim().max(20).optional().nullable(),
    sourcePrice: zod_1.z.coerce.number().min(0).max(999999999).optional().default(0),
    sourcePaymentStatus: zod_1.z.enum(["pending", "paid", "partial", "waived"]).optional().default("pending"),
    sourcingPaymentStatus: zod_1.z.enum(["paid", "pay_later"]).optional().default("pay_later"),
    salesChannel: zod_1.z.enum(["catalog", "pos_only"]).optional().default("catalog"),
    isCatalogVisible: zod_1.z.boolean().optional().default(true),
});
const receiveStockSchema = zod_1.z.object({
    quantity: zod_1.z.coerce.number().int().min(1),
    unitSourcePrice: zod_1.z.coerce.number().min(0).default(0),
    sellingPrice: zod_1.z.coerce.number().int().min(0).optional(),
    sourceId: zod_1.z.string().optional().nullable(),
    sourcedBy: zod_1.z.string().trim().max(191).optional().nullable(),
    sourceDate: zod_1.z.string().trim().max(20).optional().nullable(),
    paymentStatus: zod_1.z.enum(["pending", "paid", "partial", "waived"]).default("pending"),
    paymentMethod: zod_1.z.string().trim().max(80).optional().nullable(),
    reference: zod_1.z.string().trim().max(191).optional().nullable(),
    receiptUrl: zod_1.z.string().trim().max(1000).optional().nullable(),
...
function adminEntrantId(req, db = null) {
    ...
    if (tokenId && (!db || (db.adminUsers || []).some((admin) => String(admin.id || "") === tokenId)))
        return tokenId;
...
function syncProductSerialNumbers(db, product, incomingSerialNumbers) {
    ...
    const existingForProduct = (0, db_1.getProductSerialNumbers)(db, product.id);
    const nextKeys = new Set(nextSerialNumbers.map((item) => item.toLowerCase()));
    for (const serialNumber of nextSerialNumbers) {
        const globalExisting = (0, db_1.findSerialNumber)(db, serialNumber);
        if (globalExisting && globalExisting.productId !== product.id) {
            throw new errorHandler_1.AppError(400, `Serial number ${serialNumber} is already assigned to another product`);
        }
    }
    for (const serial of existingForProduct) {
        const key = (0, db_1.normalizeSerialNumber)(serial.serialNumber).toLowerCase();
        if (nextKeys.has(key))
            continue;
        if (serial.status !== "available") {
            throw new errorHandler_1.AppError(400, `Sold serial number ${serial.serialNumber} cannot be removed from this product`);
        }
    }
...
function ensureUniqueBarcode(db, barcodeValue, excludeProductId) {
    ...
    if (!barcode)
        return;
    const existing = (0, db_1.findProductByBarcode)(db, barcode);
    if (existing && existing.id !== excludeProductId) {
        throw new errorHandler_1.AppError(400, "Barcode is already assigned to another product");
    }
}
function normalizeSpecsPayload(db, subcategoryValue, rawSpecs) {
    ...
    const subcategory = (0, db_1.findSubcategory)(db, subcategoryValue);
    if (!subcategory)
        throw new errorHandler_1.AppError(400, "Subcategory not found");
    const nextSpecs = {};
    for (const [key, value] of Object.entries(source)) {
        const specification = (0, db_1.findSpecification)(db, String(key), subcategory.id);
        if (!specification)
            throw new errorHandler_1.AppError(400, `Unknown specification: ${String(key)}`);
        const option = (0, db_1.findSpecificationValue)(db, String(value), specification.id);
        if (!option)
            throw new errorHandler_1.AppError(400, `Invalid specification value for ${specification.name}`);
...
router.get("/sourcing/payments", (0, auth_1.requireAdminPermission)("sourcing_payments:view"), async (req, res, next) => {
    try {
        const db = await (0, db_1.readDb)();
        ...
        const products = db.products
            ...
            .map((product) => formatProduct(db, product));
        res.json(products);
    }
    catch (e) {
        next(e);
...
router.get("/:id", (0, auth_1.requireAdminPermission)("products:view"), async (req, res, next) => {
    try {
        const db = await (0, db_1.readDb)();
        const product = (0, db_1.findProductById)(db, String(req.params.id));
        if (!product)
...
router.post("/", (0, auth_1.requireAdminPermission)("products:create"), async (req, res, next) => {
    try {
        const data = productSchema.parse(req.body);
        const db = await (0, db_1.readDb)();
        if (!(0, db_1.findCategoryById)(db, data.categoryId))
            throw new errorHandler_1.AppError(400, "Category not found");
        if (!(0, db_1.findBrand)(db, data.brand))
            throw new errorHandler_1.AppError(400, "Brand not found");
        ensureUniqueBarcode(db, data.barcode);
        if (data.subcategory) {
            const subcategory = (0, db_1.findSubcategory)(db, data.subcategory, data.categoryId);
            if (!subcategory)
                throw new errorHandler_1.AppError(400, "Subcategory not found for selected category");
        }
...
router.put("/:id", requireProductUpdatePermission, async (req, res, next) => {
    try {
        ...
        if (!(0, db_1.findCategoryById)(db, nextCategoryId))
            throw new errorHandler_1.AppError(400, "Category not found");
        if (!(0, db_1.findBrand)(db, nextBrand))
            throw new errorHandler_1.AppError(400, "Brand not found");
        ensureUniqueBarcode(db, nextBarcode, product.id);
        if (nextSubcategory) {
            const resolvedSubcategory = (0, db_1.findSubcategory)(db, nextSubcategory, nextCategoryId);
        ...
        Object.assign(product, {
            ...
            ...(data.sourcingPaymentStatus !== undefined
                ? {
                    sourcingPaymentStatus: data.sourcingPaymentStatus === "paid" ? "paid" : "pay_later",
                    sourcingPaidAt: data.sourcingPaymentStatus === "paid" ? (product.sourcingPaidAt || (0, db_1.nowIso)()) : null,
...
router.post("/:id/receive-stock", (0, auth_1.requireAdminPermission)("inventory:receive_stock"), async (req, res, next) => {
    try {
        ...
        if (serialNumbers.length > data.quantity) {
            throw new errorHandler_1.AppError(400, "Serial numbers cannot be more than received quantity");
        }
        for (const serialNumber of serialNumbers) {
            const existing = (0, db_1.findSerialNumber)(db, serialNumber);
        ...
        Object.assign(product, {
            ...
            sourcePaymentStatus: data.paymentStatus,
            sourcingPaymentStatus: data.paymentStatus === "paid" ? "paid" : "pay_later",
            sourcingPaidAt: data.paymentStatus === "paid" ? now : null,
            sourcingPaidBy: data.paymentStatus === "paid" ? entrantId : null,
...
router.patch("/:id/sourcing-payment", (0, auth_1.requireAdminPermission)("sourcing_payments:update"), async (req, res, next) => {
    try {
        const data = zod_1.z.object({ paymentStatus: zod_1.z.enum(["paid", "pay_later"]).default("paid") }).parse(req.body || {});
        const db = await (0, db_1.readDb)();
        const product = (0, db_1.findProductById)(db, String(req.params.id));
        if (!product)
            throw new errorHandler_1.AppError(404, "Product not found");
...
router.delete("/:id", (0, auth_1.requireAdminPermission)("products:delete"), async (req, res, next) => {
    try {
        const db = await (0, db_1.readDb)();
        const product = (0, db_1.findProductById)(db, String(req.params.id));
        if (!product)
            throw new errorHandler_1.AppError(404, "Product not found");
        const [{ count: orderItemCount = 0 } = {}] = await (0, db_1.queryDb)("SELECT COUNT(*) AS count FROM `order_items` WHERE `product_id` = ?", [product.id]);
        const [{ count: movementCount = 0 } = {}] = await (0, db_1.queryDb)("SELECT COUNT(*) AS count FROM `inventory_movements` WHERE `product_id` = ?", [product.id]);
        const [{ count: expenseCount = 0 } = {}] = await (0, db_1.queryDb)("SELECT COUNT(*) AS count FROM `expenses` WHERE `product_id` = ?", [product.id]);
        const hasBusinessHistory = Number(orderItemCount) > 0 || Number(movementCount) > 0 || Number(expenseCount) > 0;
        await (0, db_1.executeDb)("DELETE FROM `cart_items` WHERE `product_id` = ?", [product.id]);
        ...
        await (0, db_1.executeDb)("DELETE FROM `product_images` WHERE `product_id` = ?", [product.id]);
        await (0, db_1.executeDb)("DELETE FROM `product_analytics` WHERE `product_id` = ?", [product.id]);
        await (0, db_1.executeDb)("DELETE FROM `reviews` WHERE `product_id` = ?", [product.id]);
        await (0, db_1.executeDb)("DELETE FROM `products` WHERE `id` = ?", [product.id]);
...
router.patch("/:id/toggle-featured", (0, auth_1.requireAdminPermission)("products:toggle_featured"), async (req, res, next) => {
    try {
        const db = await (0, db_1.readDb)();
        const product = (0, db_1.findProductById)(db, String(req.params.id));
        if (!product)
...
router.patch("/:id/toggle-premium", (0, auth_1.requireAdminPermission)("products:toggle_premium"), async (req, res, next) => {
    try {
        const db = await (0, db_1.readDb)();
        const product = (0, db_1.findProductById)(db, String(req.params.id));
        if (!product)
...
router.patch("/:id/toggle-stock", (0, auth_1.requireAdminPermission)("inventory:adjust"), async (req, res, next) => {
    try {
        const db = await (0, db_1.readDb)();
        const product = (0, db_1.findProductById)(db, String(req.params.id));
        if (!product)
