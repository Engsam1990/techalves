...
const ENABLE_RUNTIME_DB_MIGRATIONS = String(
  process.env.AUTO_DB_MIGRATIONS || process.env.RUNTIME_DB_MIGRATIONS || ""
...
function requireEnv(name: string, options?: { allowEmpty?: boolean }) {
...
function normalizeImageList(value: any) {
  const parsed = Array.isArray(value) ? value : parseJson(value, []);
  const seen = new Set<string>();
  return (Array.isArray(parsed) ? parsed : [])
    .map((item: any) => String(item?.url ?? item ?? "").trim())
    .filter(Boolean)
    .filter((url: string) => {
...
function normalizeStockQuantity(stockQuantity: any, inStock: any) {
  const numeric = Number(stockQuantity || 0);
  if (numeric > 0) return numeric;
  return asBool(inStock) ? 1 : 0;
}

async function tableExists(connection: mysql.Pool | mysql.PoolConnection, tableName: string) {
  const [rows] = await connection.query(`SHOW TABLES LIKE ?`, [tableName]);
  return Array.isArray(rows) && rows.length > 0;
}

async function columnExists(connection: mysql.Pool | mysql.PoolConnection, tableName: string, columnName: string) {
...
async function addColumnIfMissing(
  ...
  try {
    ...
    if (isDuplicateColumnError(error)) return false;
    throw error;
  }
}

async function ensureOrderPaymentMethodEnum(connection: mysql.Pool | mysql.PoolConnection) {
  if (!(await tableExists(connection, "orders")) || !(await columnExists(connection, "orders", "payment_method"))) return;
  ...
  const type = String((column as any)?.Type || "").toLowerCase();
  const supportsAllMethods = requiredMethods.every((method) => type.includes(`'${method}'`));

  if (!supportsAllMethods) {
    await connection.query(
      `ALTER TABLE ${qid("orders")} MODIFY ${qid("payment_method")} enum('cash_on_delivery','mpesa','bank_transfer','cash','card','other') NOT NULL`
    );
  }
}
...
async function ensureDataEntryColumns(connection: mysql.Pool | mysql.PoolConnection, tableName: string) {
...
async function ensureProductSourcingColumns(connection: mysql.Pool | mysql.PoolConnection) {
...
async function ensureProductImagesColumn(connection: mysql.Pool | mysql.PoolConnection) {
  if (!(await tableExists(connection, "products"))) return;
...
async function ensureExpensesTable(connection: mysql.Pool | mysql.PoolConnection) {
  await connection.query(`
...
async function ensureProductSourcesTable(connection: mysql.Pool | mysql.PoolConnection) {
  await connection.query(
    "CREATE TABLE IF NOT EXISTS " + qid("product_sources") + " (" +
      qid("id") + " varchar(191) NOT NULL," +
...
async function ensureExpenseCategoriesTable(connection: mysql.Pool | mysql.PoolConnection) {
  await connection.query(
    "CREATE TABLE IF NOT EXISTS " + qid("expense_categories") + " (" +
      qid("id") + " varchar(191) NOT NULL," +
...
async function ensureFinanceSchemaColumns(connection: mysql.Pool | mysql.PoolConnection) {
...
async function ensureInventoryMovementTypeEnum(connection: mysql.Pool | mysql.PoolConnection) {
  if (!(await tableExists(connection, "inventory_movements")) || !(await columnExists(connection, "inventory_movements", "type"))) return;
  ...
  if (!supportsAllTypes) {
    await connection.query(
      `ALTER TABLE ${qid("inventory_movements")} MODIFY ${qid("type")} enum('initial_stock','manual_adjustment','order_deducted','order_restored','pos_sale','customer_checkout','supplier_payment') NOT NULL`
    );
  }
}
async function ensureOrderItemSnapshotColumns(connection: mysql.Pool | mysql.PoolConnection) {
  if (!(await tableExists(connection, "order_items"))) return;
...
async function fetchOptionalTable(connection: mysql.Pool | mysql.PoolConnection, tableName: string, sql: string) {
  if (!(await tableExists(connection, tableName))) return [];
...
export async function executeDb(sql: string, params: any[] = []) {
  await getPool().execute(sql, params);
}

async function fetchTable(connection: mysql.Pool | mysql.PoolConnection, sql: string) {
  const [rows] = await connection.query(sql);
  return Array.isArray(rows) ? rows : [];
}

async function runRuntimeDbMigrations(connection: mysql.Pool | mysql.PoolConnection) {
  if (!runtimeDbMigrationsEnabled()) return;
...
export async function readDb() {
  ...
  const brandNameByLookup = new Map<string, string>();
  ...
  for (const row of raw.orderItems) {
    ...
    orderItemsByOrderId.set(
      String(row.order_id),
      (orderItemsByOrderId.get(String(row.order_id)) || []).concat({
        id: String(row.id),
        productId: String(row.product_id),
        ...
        serialNumbers: Array.isArray(serialValues)
          ? serialValues.map((value: any) => normalizeSerialNumber(value?.serialNumber ?? value)).filter(Boolean)
          : [],
      })
    );
  }
  ...
  return {
    ...
    products: raw.products.map((row) => {
      ...
      const serialNumbers = Array.isArray(storedSerialNumbers)
        ? storedSerialNumbers.map((item: any) => normalizeSerialNumber(item?.serialNumber ?? item)).filter(Boolean)
        : [];
    ...
    adminUsers: raw.adminUsers.map((row) => ({
      ...
      permissions: parseJson(row.permissions_json, null),
      permissionsJson: row.permissions_json ?? null,
      isActive: asBool(row.is_active),
      createdAt: toIso(row.created_at),
      updatedAt: toIso(row.updated_at),
    })),
...
async function executeMany(connection: mysql.PoolConnection, sql: string, rows: any[][]) {
...
function stableGeneratedId(prefix: string, parts: any[]) {
  const hash = crypto.createHash("sha1").update(parts.map((part) => String(part ?? "")).join("|"), "utf8").digest("hex").slice(0, 32);
  return `${prefix}-${hash}`.slice(0, 191);
}

function slugify(value: any, fallback = "item") {
...
function mergeByStableId(items: any[], _defaults: any[] = []) {
...
function normalizeExpenseCategories(items: any[]) {
  const byId = new Map<string, any>();
  ...
  for (const item of Array.isArray(items) ? items : []) {
    ...
    if (!explicitName && !explicitSlug) continue;

    const slug = slugify(explicitSlug || explicitName, "other");
    if (slug === "supplier") continue;
...
function normalizeCustomerUsersAndReferences(db: any) {
  ...
  return (Array.isArray(db.customerUsers) ? db.customerUsers : []).filter((item: any) => String(item?.id || "").trim());
}

function normalizeProductSources(items: any[]) {
...
export async function writeDb(db: any) {
  ...
  try {
    ...
    await executeMany(connection, insertSql("admin_users", ["id", "email", "password", "name", "role", "permissions_json", "is_active", "created_at", "updated_at", "data_entrant", "entry_date"]), (db.adminUsers || []).map((item: any) => [
      item.id,
      item.email,
      item.password,
      item.name,
      item.role || "admin",
      item.role === "super_admin" ? null : JSON.stringify(Array.isArray(item.permissions) ? item.permissions : parseJson(item.permissionsJson, [])),
    ...
    await executeMany(connection, insertSql("customer_users", ["id", "full_name", "email", "phone", "password_hash", "is_active", "created_at", "updated_at"]), normalizedCustomerUsers.map((item: any) => [
      item.id,
      item.fullName,
      item.email,
      item.phone || null,
    ...
    await executeMany(connection, insertSql("categories", ["id", "slug", "name", "description", "image_url", "icon", "sort_order", "is_active", "created_at"]), (db.categories || []).map((item: any) => [
      item.id,
      item.slug,
      item.name,
    ...
    await executeMany(connection, insertSql("products", ["id", "slug", "name", "category_id", "subcategory", "brand", "source_id", "barcode", "price", "original_price", "condition", "description", "specs", "warranty_text", "in_stock", "featured", "premium", "rating", "review_count", "images", "created_at", "stock_quantity", "total_stock_received", "serial_numbers", "created_by_admin_user_id", "updated_by_admin_user_id", "sourced_from", "sourced_by", "source_date", "source_price", "source_payment_status", "sales_channel", "is_catalog_visible", "sourcing_payment_status", "sourcing_paid_at", "sourcing_paid_by", "data_entrant", "entry_date"]), (db.products || []).map((item: any) => {
      ...
      const serialNumbers = Array.isArray(item.serialNumbers)
        ? item.serialNumbers.map((serial: any) => normalizeSerialNumber(serial?.serialNumber ?? serial)).filter(Boolean)
        : [];
    ...
    const productIdsForImageMirror = (db.products || []).map((product: any) => String(product.id || "").trim()).filter(Boolean);
    if (productIdsForImageMirror.length) {
      await connection.query(
        "DELETE FROM " + qid("product_images") + " WHERE " + qid("product_id") + " IN (" + productIdsForImageMirror.map(() => "?").join(",") + ")",
        productIdsForImageMirror
      );
    }

    await executeMany(connection, insertSql("product_images", ["id", "product_id", "url", "sort_order", "data_entrant", "entry_date"]), (db.products || []).flatMap((product: any) =>
      normalizeImageList(product.images).map((url: string, index: number) => [
    ...
    await executeMany(connection, insertSql("blog_posts", ["id", "slug", "title", "excerpt", "content_html", "image_url", "author", "category", "read_time", "published_at", "is_published"]), (db.blogPosts || []).map((item: any) => [
      item.id,
      item.slug,
      item.title,
      item.excerpt,
    ...
    await executeMany(connection, insertSql("carts", ["id", "customer_id", "created_at", "updated_at"]), (db.carts || []).map((item: any) => [
      item.id,
      item.customerId,
      sqlDate(item.createdAt),
      sqlDate(item.updatedAt || item.createdAt),
    ...
    await executeMany(connection, insertSql("cart_items", ["id", "cart_id", "product_id", "quantity", "created_at", "updated_at"]), (db.carts || []).flatMap((cart: any) =>
    ...
    await executeMany(connection, insertSql("order_items", ["id", "order_id", "product_id", "product_name", "product_brand", "product_barcode", "source_id", "source_name", "unit_source_price", "total_source_price", "unit_selling_price", "total_selling_price", "vat_rate", "vat_amount", "serial_numbers", "quantity", "unit_price", "total_price", "data_entrant", "entry_date"]), (db.orders || []).flatMap((order: any) =>
      (order.items || []).map((item: any) => [
        ...
        Number(item.vatRate || 0),
        Number(item.vatAmount || 0),
        JSON.stringify(Array.isArray(item.serialNumbers) ? item.serialNumbers.map((serial: any) => normalizeSerialNumber(serial?.serialNumber ?? serial)).filter(Boolean) : []),
        Number(item.quantity || 0),
    ...
    await executeMany(connection, `UPDATE ${qid("products")} SET ${qid("stock_quantity")} = ?, ${qid("in_stock")} = ?, ${qid("total_stock_received")} = ?, ${qid("serial_numbers")} = ? WHERE ${qid("id")} = ?`, (db.products || []).map((item: any) => {
      const stockQuantity = Number(item.stockQuantity || 0);
      const totalStockReceived = Number(item.totalStockReceived ?? stockQuantity);
      const serialNumbers = Array.isArray(item.serialNumbers)
        ? item.serialNumbers.map((serial: any) => normalizeSerialNumber(serial?.serialNumber ?? serial)).filter(Boolean)
    ...
    await executeMany(connection, insertSql("inventory_movements", ["id", "product_id", "order_id", "admin_user_id", "type", "quantity_change", "quantity_before", "quantity_after", "note", "created_at", "data_entrant", "entry_date"]), (db.inventoryMovements || []).map((item: any) => [
      item.id,
      item.productId,
      item.orderId || null,
    ...
    await executeMany(connection, insertSql("product_analytics", ["id", "product_id", "event", "metadata", "ip_address", "user_agent", "created_at"]), (db.productAnalytics || []).map((item: any) => [
      item.id,
      item.productId,
      item.event,
    ...
    await executeMany(connection, insertSql("contact_messages", ["id", "full_name", "email", "phone", "subject", "message", "status", "created_at"]), (db.contactMessages || []).map((item: any) => [
      item.id,
      item.fullName,
      item.email,
      item.phone || null,
    ...
    await executeMany(connection, insertSql("newsletter_subscribers", ["id", "email", "source", "created_at"]), (db.newsletterSubscribers || []).map((item: any) => [
      item.id,
      item.email,
      item.source || null,
      sqlDate(item.createdAt),
    ...
    await executeMany(connection, insertSql("expenses", ["id", "details", "category", "amount", "expense_date", "order_id", "data_entrant", "entry_date", "created_at", "updated_at", "category_id", "product_id", "source_id", "order_item_id", "payment_status", "payment_method", "reference", "description", "created_by_admin_user_id"]), (db.expenses || []).map((item: any) => [
      item.id,
    ...
    await executeMany(connection, insertSql("password_reset_tokens", ["id", "customer_id", "token", "expires_at", "used_at", "created_at"]), (db.passwordResetTokens || []).map((item: any) => [
      item.id,
      item.customerId,
      item.token,
    ...
    await executeMany(connection, insertSql("reviews", ["id", "product_id", "customer_id", "rating", "comment", "is_approved", "created_at", "updated_at"]), (db.reviews || []).map((item: any) => [
      item.id,
      item.productId,
      item.customerId,
      Number(item.rating || 0),
...
export async function updateDb(fn: any) {
...
export function contains(value: any, query: any) {
  if (!query) return true;
  return String(value ?? "").toLowerCase().includes(String(query).toLowerCase());
}

export function findCategoryById(db: any, id: string) {
  return db.categories.find((item: any) => item.id === id) || null;
}

export function findCategoryBySlug(db: any, slug: string) {
  return db.categories.find((item: any) => item.slug === slug) || null;
}

export function findProductById(db: any, id: string) {
  return db.products.find((item: any) => item.id === id) || null;
}

export function findProductBySlug(db: any, slug: string) {
  return db.products.find((item: any) => item.slug === slug) || null;
}

export function findProductByBarcode(db: any, barcode: string) {
  const needle = String(barcode || "").trim().toLowerCase();
  if (!needle) return null;
  return db.products.find((item: any) => String(item.barcode || "").trim().toLowerCase() === needle) || null;
}

export function normalizeSerialNumber(value: any) {
  return String(value ?? "").trim();
}

function buildSerialUsageMap(db: any, options?: { excludeOrderId?: string | null; excludeOrderIds?: string[] }) {
  const excludedOrderIds = new Set(
    [options?.excludeOrderId, ...(options?.excludeOrderIds || [])]
      .map((value) => String(value ?? "").trim())
      .filter(Boolean)
  );

  const usage = new Map<string, Map<string, any>>();
  ...
  for (const order of db.orders || []) {
    ...
    if (!["confirmed", "processing", "completed", "delivered"].includes(status)) continue;
...
export function getProductSerialNumbers(db: any, productId: string, options?: { excludeOrderId?: string | null; excludeOrderIds?: string[] }) {
...
export function getAvailableSerialNumbers(db: any, productId: string, options?: { excludeOrderId?: string | null; excludeOrderIds?: string[] }) {
  return getProductSerialNumbers(db, productId, options).filter((item: any) => item.status === "available");
}

export function findSerialNumber(db: any, serialNumber: string) {
...
export function getOrCreateCart(db: any, customerId: string) {
  let cart = db.carts.find((item: any) => item.customerId === customerId);
  if (!cart) {
    const now = nowIso();
...
export function sortByDateDesc(items: any[], field = "createdAt") {
  return [...items].sort((a, b) => new Date(b?.[field] || 0).getTime() - new Date(a?.[field] || 0).getTime());
}

export function sortByDateAsc(items: any[], field = "createdAt") {
...
export function normalizeProduct(product: any) {
  ...
  if (product.reviewCount === undefined || product.reviewCount === null) product.reviewCount = 0;
  return product;
}

export function findBrand(db: any, value: string) {
  const needle = String(value || "").trim().toLowerCase();
  if (!needle) return null;
  return (db.brands || []).find((item: any) =>
...
export function findSubcategory(db: any, value: string, categoryId?: string | null) {
  const needle = String(value || "").trim().toLowerCase();
  if (!needle) return null;
  return (db.subcategories || []).find((item: any) => {
...
export function findSpecification(db: any, value: string, subcategoryValue?: string | null) {
  const needle = String(value || "").trim().toLowerCase();
  if (!needle) return null;
  const subcategory = subcategoryValue ? findSubcategory(db, String(subcategoryValue)) : null;
  return (db.specifications || []).find((item: any) => {
    if (subcategory && item.subcategoryId !== subcategory.id) return false;
    return String(item.id || "").toLowerCase() === needle || String(item.name || "").toLowerCase() === needle;
  }) || null;
}

export function findSpecificationValue(db: any, value: string, specificationId?: string | null) {
  const needle = String(value || "").trim().toLowerCase();
  if (!needle) return null;
  return (db.specificationValues || []).find((item: any) => {
    if (specificationId && String(item.specificationId) !== String(specificationId)) return false;
    return String(item.id || "").toLowerCase() === needle || String(item.value || "").toLowerCase() === needle;
  }) || null;
}

export function resolveProductSpecs(db: any, specs: any, subcategoryValue?: string | null) {
  ...
  for (const [rawKey, rawValue] of Object.entries(source)) {
    ...
    if (!specification) {
      ...
      if (keyText && valueText) display[keyText] = valueText;
      continue;
    }
...
export async function compareSecret(stored: string, incoming: string) {
...
export function recalcProductReviewStats(db: any, productId: string) {
  const approved = db.reviews.filter((item: any) => item.productId === productId && item.isApproved !== false);
  const count = approved.length;
  const avg = count ? approved.reduce((sum: number, item: any) => sum + Number(item.rating || 0), 0) / count : 0;
  const product = findProductById(db, productId);
  if (product) {
