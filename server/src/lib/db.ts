...
export async function writeDb(db: any) {
  ...
  try {
    ...
    await executeMany(connection, insertSql("admin_users", ["id", "email", "password", "name", "role", "is_active", "created_at", "updated_at", "data_entrant", "entry_date"]), (db.adminUsers || []).map((item: any) => [
      item.id,
      item.email,
      item.password,
      item.name,
