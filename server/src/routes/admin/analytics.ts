...
function requireDashboardOverviewPermission(req, _res, next) {
  try {
    if (DASHBOARD_OVERVIEW_PERMISSIONS.some((permission) => hasAdminPermission(req.adminUser, permission))) {
      return next();
    }
...
function requireInventoryReportPermission(req, _res, next) {
  try {
    if (INVENTORY_REPORT_PERMISSIONS.some((permission) => hasAdminPermission(req.adminUser, permission))) {
      return next();
    }
    throw new AppError(403, "You do not have permission to perform this admin action");
  } catch (error) {
...
function buildFinanceReport(db, range, req = null) {
  const canViewAll = !req || hasAdminPermission(req.adminUser, "finance:view_all");
