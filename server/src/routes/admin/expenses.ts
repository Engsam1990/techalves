...
router.get("/", requireAdminPermission("expenses:view"), async (req, res, next) => {
  try {
    ...
    const items = (db.expenses || [])
      ...
      .filter((expense) => hasAdminPermission(req.adminUser, "expenses:view_all") || isOwnAdminRecord(req, expense))
...
router.post("/", requireAdminPermission("expenses:create"), async (req, res, next) => {
  try {
