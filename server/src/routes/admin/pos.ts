...
const paymentMethods = ["cash", "mpesa", "card", "bank_transfer", "other"] as const;
const methodsRequiringTransactionReference = new Set(["mpesa", "card", "bank_transfer"]);
const sourcePaymentStatuses = ["pending", "paid", "partial", "waived"] as const;
const WALK_IN_CUSTOMER_ID = "pos-walkin-customer";
const WALK_IN_CUSTOMER_EMAIL = "pos-walkin@techalves.local";

const instantProductSchema = z.object({
  name: z.string().trim().min(1).max(200),
  categoryId: z.string().uuid({ message: "Category is required for instant-sale products" }),
  subcategory: z.string().trim().max(191).optional().nullable(),
  brand: z.string().trim().min(1, "Brand is required for instant-sale products").max(100),
  barcode: z.string().trim().max(120).optional().nullable(),
  condition: z.enum(["new", "refurbished", "ex-uk"]).default("new"),
  description: z.string().trim().max(2000).optional().nullable(),
  warrantyText: z.string().trim().max(191).optional().nullable(),
  sourceId: z.string().trim().min(1, "Supplier/source is required for instant-sale products"),
  sourcePrice: z.coerce.number().int().positive("Source price is required for instant-sale products"),
  sellingPrice: z.coerce.number().int().positive("Selling price is required"),
  sourcePaymentStatus: z.enum(sourcePaymentStatuses, { message: "Source payment status is required" }),
});
...
const posCheckoutSchema = z.object({
  ...
  sourcePaymentMethod: z.enum(paymentMethods).optional().nullable(),
  sourcePaymentReference: z.string().trim().max(120).optional().nullable(),
});
...
function ensureCollections(db) {
  ...
  if (!Array.isArray(db.productSerialNumbers)) db.productSerialNumbers = [];
  if (!Array.isArray(db.productSources)) db.productSources = [];
  if (!Array.isArray(db.expenseCategories)) db.expenseCategories = [];
  if (!Array.isArray(db.expenses)) db.expenses = [];
  if (!Array.isArray(db.customerUsers)) db.customerUsers = [];
...
function ensurePosWalkInCustomer(db, now) {
  ...
  const existingByEmail = (db.customerUsers || []).find(
    (item) => String(item.email || "").trim().toLowerCase() === WALK_IN_CUSTOMER_EMAIL
  );
  if (existingByEmail) {
...
function assertAndReserveSerials(db, product, orderItem, serialNumbers, order, soldAt) {
  ...
  if (selectedSerials.length !== Number(orderItem.quantity || 0)) {
    throw new AppError(400, `${product.name} requires ${orderItem.quantity} serial number(s)`);
  }

  const availableByKey = new Map(getAvailableSerialNumbers(db, product.id, { excludeOrderId: order.id }).map((item) => [normalizeSerialNumber(item.serialNumber).toLowerCase(), item]));
  const assigned = [];
  for (const serialNumber of selectedSerials) {
    const serial = availableByKey.get(serialNumber.toLowerCase());
    if (!serial) throw new AppError(400, `Serial number ${serialNumber} is not available for ${product.name}`);
    serial.status = "sold";
...
function createInstantProduct(db, instant, quantity, serialNumbers, adminUserId, entrantName, now) {
  const id = createId();
  const category = findCategoryById(db, instant.categoryId);
  if (!category) throw new AppError(400, "Category is required for instant-sale products");

  const subcategory = instant.subcategory ? findSubcategory(db, instant.subcategory, category.id) : null;
  if (instant.subcategory && !subcategory) throw new AppError(400, "Selected subcategory does not belong to the selected category");

  const brand = findBrand(db, instant.brand);
  if (!brand) throw new AppError(400, "Brand is required for instant-sale products");

  const source = findSource(db, instant.sourceId);
  if (!source || source.isActive === false) throw new AppError(400, "Supplier/source is required for instant-sale products");

  const sourcePrice = Number(instant.sourcePrice || 0);
  const sellingPrice = Number(instant.sellingPrice || 0);
  if (sourcePrice <= 0) throw new AppError(400, "Source price is required for instant-sale products");
  if (sellingPrice <= 0) throw new AppError(400, "Selling price is required for instant-sale products");
...
router.post("/checkout", async (req, res, next) => {
  try {
    ...
    if (paymentNeedsTransactionReference(data.paymentMethod, data.paymentStatus) && !transactionReference) {
      throw new AppError(400, "Transaction reference is required for paid M-Pesa, card, and bank transfer sales");
    }
    ...
    const items = data.items.map((rawItem) => {
      ...
      if (rawItem.instantProduct) {
        ...
        product = findProductById(db, rawItem.productId);
      }

      if (!product) throw new AppError(400, "One or more scanned products no longer exist");
      ...
      if (Number(rawItem.quantity) > Number(product.stockQuantity || 0)) {
        throw new AppError(400, `${product.name} only has ${product.stockQuantity} unit(s) left`);
      }
    ...
    if (data.vatEnabled) checkoutNote.push(`VAT ${vatRate}% applied`);
    if (data.note) checkoutNote.push(data.note);
