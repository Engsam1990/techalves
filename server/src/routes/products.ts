...
import { readDb, findCategoryById, findCategoryBySlug, findBrand, findSubcategory, resolveProductSpecs, normalizeProduct, sortByDateDesc } from "../lib/db";
...
router.get("/", async (req, res, next) => {
  try {
    ...
    let items = db.products.filter(isPublicProduct).map((item) => normalizeProduct({ ...item }));

    if (query.category) {
      const category = findCategoryBySlug(db, query.category);
      items = category ? items.filter((item) => item.categoryId === category.id) : [];
    }

    const featuredFilter = parseBooleanFilter(query.featured);
    if (featuredFilter !== null) {
      items = items.filter((item) => normalizeProduct(item).featured === featuredFilter);
    }

    const premiumFilter = parseBooleanFilter(query.premium);
    if (premiumFilter !== null) {
      items = items.filter((item) => normalizeProduct(item).premium === premiumFilter);
    }

    if (query.brand) {
      const brands = query.brand.split(",").map((item) => item.trim().toLowerCase());
      items = items.filter((item) => {
        const displayBrand = findBrand(db, item.brand)?.name ?? item.brand;
    ...
    if (query.minPrice !== undefined) items = items.filter((item) => Number(item.price) >= Number(query.minPrice));
    if (query.maxPrice !== undefined) items = items.filter((item) => Number(item.price) <= Number(query.maxPrice));

    if (query.q) {
      const q = String(query.q).toLowerCase();
      items = items.filter((item) => {
        const brand = findBrand(db, item.brand)?.name ?? item.brand;
        const subcategory = findSubcategory(db, item.subcategory, item.categoryId)?.name ?? item.subcategory;
        const specText = Object.values(resolveProductSpecs(db, item.specs, item.subcategory).display || {}).join(" ");
        return [item.name, brand, subcategory, item.description, specText].some((value) => String(value || "").toLowerCase().includes(q));
      });
    }

    if (query.sort === "price-asc") items.sort((a, b) => Number(a.price) - Number(b.price));
    else if (query.sort === "price-desc") items.sort((a, b) => Number(b.price) - Number(a.price));
    else if (query.sort === "rating") items.sort((a, b) => Number(b.rating || 0) - Number(a.rating || 0));
    else items = sortByDateDesc(items, "createdAt");
