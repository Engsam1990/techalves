...
const AdminProductForm = () => {
  ...
  const buildCreatePayload = () => ({ ...form, slug: form.slug || slugify(form.name), barcode: form.barcode.trim() || null, originalPrice: form.originalPrice || undefined, subcategory: form.subcategory || undefined, warrantyText: form.warrantyText || undefined, stockQuantity: Number(form.stockQuantity) || 0, premium: Boolean(form.premium), images: form.images.filter(Boolean), specs: filteredSpecs(), sourceId: form.sourceId || null, sourcedBy: undefined, sourcePaymentStatus: form.sourcePaymentStatus || "pending", sourcingPaymentStatus: form.sourcePaymentStatus === "paid" ? "paid" : "pay_later", serialNumbers: parseSerialNumbers(form.serialNumbersText) });
  const buildSectionPayload = (section: ProductSection) => {
    ...
    if (section === "sources_supplier") return { sourceId: form.sourceId || null, sourcedBy: undefined, sourceDate: form.sourceDate || null, sourcePrice: Number(form.sourcePrice || 0), sourcePaymentStatus: form.sourcePaymentStatus || "pending", sourcingPaymentStatus: form.sourcePaymentStatus === "paid" ? "paid" : "pay_later" };
