...
const COLORS = ["#dc2626", "#16a34a", "#d97706", "#2563eb", "#7c3aed"];
...
const InventoryDashboard = () => {
  ...
  const largestCategoryShare = largestCategory && summary?.inventoryValue
    ...
    : 0;
  const quickRestockItems = [...(issues?.lowStockProducts || []), ...(issues?.outOfStockProducts || [])]
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-6 items-start xl:grid-cols-3">
            ...
            <CardContent className="space-y-4">
              {[
                ...
                <div key={item.label} className="flex items-center justify-between gap-3">
                  ...
                  {loading ? <Skeleton className="h-5 w-24" /> : <p className="text-sm font-semibold text-foreground">{item.value}</p>}
                </div>
              ))}
            </CardContent>
          </Card>
              ...
              <ResponsiveContainer width="100%" height={240}>
                <PieChart>
                  <Pie
                    ...
                    labelLine={false}
                  >
                    {stockHealthBreakdown.map((_: { status: string; count: number; units: number; value: number; label: string }, index: number) => (
      ...
      <div className="columns-1 space-y-2 [column-gap:1.5rem] xl:columns-2">
        <IssueListCard
