...
async function audit(label, envPath) {
  ...
  console.log(JSON.stringify({ label, envPath, productMatches, orderMatches }, null, 2));
  await pool.end();
}

(async () => {
  await audit("production", "./server/.env");
  await audit("development", "./server/.env.development");
})().catch((error) => {
  console.error(error);
  process.exit(1);
});
